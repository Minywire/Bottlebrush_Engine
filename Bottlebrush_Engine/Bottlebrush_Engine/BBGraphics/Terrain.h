...
class Terrain {
   ...
   public:
    ...
    /// <p>
    /// The \c GetElements function retrieves the generated list of vertex
    /// indices for the generated terrain mesh. <p> The return value is a vector
    /// container object containing the index positions describing the vertex
    /// positions for the generated terrain.
    ...
    /// <p>
    /// The \c GetHeight function evaluates the height of the terrain in the \a
    /// y directional plane at the given \a x and \a z co-ordinates. The values
    /// supplied by the \c x and \c z parameters should be in local positional
    /// space.
    /// <p>
    /// The height is evaluated using the barycentric co-ordinate method to
    /// obtain the local cell position within a grid, and thus obtain the
